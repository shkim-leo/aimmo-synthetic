...
def main():
    ...
    def _save_image(image_name, bgr_image):
        ...
        cv2.imwrite(str(cfg.RESULTS_PATH / f"{image_name}"), rgb_image, [cv2.IMWRITE_JPEG_QUALITY, 85])
...
def parse_args():
    ...
    parser.add_argument("--images", type=str, help="Path to directory with .jpg images to get predictions for.")
    parser.add_argument(
        "--masks",
        type=str,
    ...
    parser.add_argument(
        "--original-size", action="store_true", default=False, help="Resize predicted image back to the original size."
    )
