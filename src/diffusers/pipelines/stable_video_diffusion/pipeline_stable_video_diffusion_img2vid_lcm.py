...
class StableVideoDiffusionLCMPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        denoised = denoised.to(image_embeddings.dtype)
        if not output_type == "latent":
            # cast back to fp16 if needed
            if needs_upcasting:
                self.vae.to(dtype=torch.float16)
            frames = self.decode_latents(denoised, num_frames, decode_chunk_size)
