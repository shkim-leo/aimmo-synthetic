...
class StableVideoDiffusionVid2VidPipeline(DiffusionPipeline):
    r"""
    ...
    def prepare_latents(self, video, batch_size, timestep, dtype, device, generator, latents=None):
    ...
    def __call__(
        ...
        video: Union[List[torch.Tensor], List[np.ndarray], List[PIL.Image.Image]],
        height: int = 576,
        width: int = 1024,
        strength: float = 0.8,
        ...
        image_embeddings = [
            self._encode_image(frame, device, num_videos_per_prompt, self.do_classifier_free_guidance)
            for frame in video
        ]

        channel_0, channel_1, channel_2 = image_embeddings[0].shape
        image_embeddings = torch.stack(image_embeddings, dim=1).view(channel_0, -1, channel_2)
        ###
