...
class CustomDataset(Dataset):
    ...
    def get_batch(self, idx):
        image_path, caption = self.dataset[idx][0], self.dataset[idx][1]
        image = Image.open(image_path).convert("RGB")
        pixel_value = self.transforms(image)
        return pixel_value, caption

    def __getitem__(self, idx):
        while True:
            try:
                pixel_value, caption = self.get_batch(idx)
                break
        ...
        sample = dict(pixel_values=pixel_value, input_ids=caption)
        return sample
...
def log_validation(vae, unet, args, accelerator, weight_dtype, step, validation_prompts, name="target"):
...
def main(args):
    ...
    train_dataset = CustomDataset(args.train_ann_path, args.resolution, tokenizer)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                image, input_ids = batch["pixel_values"], batch["input_ids"]
                image = image.to(accelerator.device, non_blocking=True)
                input_ids = input_ids.to(accelerator.device, non_blocking=True)
                pixel_values = image.to(dtype=weight_dtype)
                # input_ids = input_ids.to(dtype=weight_dtype)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if args.validation_prompts is not None and global_step % args.validation_steps == 0:
                        ...
                        log_validation(
                            vae, unet, args, accelerator, weight_dtype, global_step, args.validation_prompts, "online"
                        )
