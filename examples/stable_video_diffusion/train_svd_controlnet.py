...
from diffusers.models.unet_spatio_temporal_condition_controlnet import UNetSpatioTemporalConditionControlNetModel
from diffusers.pipelines.stable_video_diffusion.pipeline_stable_video_diffusion_controlnet import (
    StableVideoDiffusionControlNetPipeline,
)
from diffusers.models.controlnet_svd import ControlNetSVDModel
...
class CustomDataset(Dataset):
    def __init__(
        self,
        csv_path,
        sample_size=(320, 512),  # height, width
        sample_stride=1,
        sample_n_frames=16,
    ):
    ...
    def get_batch(self, idx):
        folder_path = self.dataset[idx][0]
        ...
        for idx in batch_index:
            ...
            control_frame = control_frame.resize((frame.width, frame.height))

            control_pixel_values.append(control_frame)
        ...
        return pixel_values, control_pixel_values
    ...
    def __getitem__(self, idx):
        while True:
            try:
                pixel_values, control_pixel_values = self.get_batch(idx)
                break
        ...
        control_pixel_values = self.conditioning_image_transforms(control_pixel_values)

        sample = dict(pixel_values=pixel_values, control_pixel_values=control_pixel_values)
        return sample
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                controlnet_image = batch["control_pixel_values"].to(latents.device)
