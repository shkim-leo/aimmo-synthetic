...
from diffusers import (
    AutoencoderKL,
    DDPMScheduler,
    StableDiffusionXLUnclipImg2ImgPipeline,
...
class Collater(object):
    def __init__(self, tokenizers, text_encoders, proportion_empty_prompts, caption_column, is_train=True):
    ...
    def encode_prompt(self, examples):
        prompt_embeds_list = []
        prompt_batch = [example[self.caption_column] for example in examples]
    ...
    def __call__(self, examples):
        ...
        text_encode_result = self.encode_prompt(examples)
        prompt_embeds = text_encode_result["prompt_embeds"]
        prompt_embeds = torch.stack([torch.tensor(prompt_embed) for prompt_embed in prompt_embeds])
        pooled_prompt_embeds = text_encode_result["pooled_prompt_embeds"]
        pooled_prompt_embeds = torch.stack(
            [torch.tensor(pooled_prompt_embed) for pooled_prompt_embed in pooled_prompt_embeds]
        )
...
def noise_image_embeddings(
    ...
    noise_level = noise_level.to(image_embeds.device)
...
def prepare_latents(
    vae, scheduler, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None
):
...
def main(args):
    ...
    image_normalizer = StableUnCLIPImageNormalizer.from_pretrained(
        os.path.join(args.pretrained_model_name_or_path, "image_normalizer")
    )
    image_noising_scheduler = DDPMScheduler.from_pretrained(
        os.path.join(args.pretrained_model_name_or_path, "image_noising_scheduler")
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                #     weight_dtype,
                #     accelerator.device,
                #     torch.Generator(device=accelerator.device).manual_seed(args.seed) if args.seed else None,
                #     None,
                # )
                ...
                if args.noise_level >= 1000:
                    train_noise = random.randint(0, 999)
                else:
                    train_noise = args.noise_level
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                pipeline = StableDiffusionXLUnclipImg2ImgPipeline.from_pretrained(
                    args.pretrained_model_name_or_path,
                    vae=vae,
