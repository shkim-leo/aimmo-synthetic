...
def log_validation(
    vae, text_encoder, tokenizer, unet, controlnet, args, accelerator, weight_dtype, global_step, epoch, val_cnt=8
):
    ...
    annotations = []
    with open(os.path.join(args.train_data_dir, "train.jsonl"), "r", encoding="utf-8") as f:
        for line in f:
            annotations.append(json.loads(line))

    for idx, anno in enumerate(annotations[:val_cnt]):
        with open(os.path.join(args.train_data_dir, anno["annotation"]), "r") as f:
            annotation = json.load(f)

        image_path = os.path.join(args.train_data_dir, anno["image"])
        control_path = os.path.join(args.train_data_dir, anno["conditioning_images"])
        caption = anno["text"]
        ...
        iter_cnt = 1000
        try:
            while select_idx is None and iter_cnt > 0:
            ...
            if select_idx is None:
                raise Exception("selected vehicle is None")
        ...
        with torch.autocast("cuda"):
            image = pipeline(
                prompt=caption,
                image=val_image,
                control_image=val_control_image,
                mask_image=mask,
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                image_path = os.path.join(self.base_path, self.annotations[idx]["image"])
                image = Image.open(image_path).convert("RGB")
                pil_image = self.transforms(image)
                pixel_value = self.transforms_post(pil_image)

                con_path = os.path.join(self.base_path, self.annotations[idx]["conditioning_images"])
                condition_image = Image.open(con_path).convert("RGB")
                conditioning_pixel_values = self.conditioning_image_transforms(condition_image)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if args.validation_prompts is not None and epoch % args.validation_epochs == 0:
                ...
                log_validation(
                    vae, text_encoder, tokenizer, unet, controlnet, args, accelerator, weight_dtype, global_step, epoch
                )
